...
### 3.3 Set up the low-level parameters and define the distributions (modify FORTRAN source files).
...
```bash
python3 -m pytest -vv # verbose mode for a full report
```
  ...
  - **`dudi.api.batch_over_points_sources(points, sources, tnow)`**
    ...
    - **Output**:
      - `ndarray` of shape `(n_points, 2)` where the densities from all
        sources are **contributions from different sources are summed for each point**.
...
### 7.1 Rebuilding the Fortran bridge (release and debug modes)
...
### 7.2 Rebuilding or modifying Fortran sources
...
### 7.4 Commiting to GitHub

The ci.yml includes a test run performed on pushes. This test run requires the set up for Io in the module
const.f90. Therefore, when commiting, make sure to copy to the module const.f90 the settings described in the main README for the Example 3 -- a volcanic eruption on Io.
